...
(
    ...
    birth_date      DATE,
    death_date      DATE CHECK (birth_date IS NULL OR death_date > birth_date),
    start_career    DATE,
    end_career      DATE CHECK (start_career IS NULL OR end_career > start_career),
    sex             CHAR(1)
...
-- Создание таблицы tv_series
CREATE TABLE IF NOT EXISTS tv_series
(
    id         INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    year_start INT
        CONSTRAINT tv_series_year_start_positive CHECK (year_start > 0) NOT NULL,
...
(
    id         INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    type       TEXT
        CONSTRAINT type_check CHECK (type = 'movie' OR type = 'series') UNIQUE NOT NULL
);

-- Создание таблицы content_type
CREATE TABLE IF NOT EXISTS content_content_type
(
...
-- Роли по умолчанию
INSERT INTO role_data (name) VALUES ('actor'), ('director'), ('producer'), ('writer'), ('operator'), ('composer'), ('editor');
